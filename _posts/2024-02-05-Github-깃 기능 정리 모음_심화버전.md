...
- git branch -r[v] : 원격 저장소에 있는 브랜치를 보고 싶을때 사용 (-v옵션 : 커밋 요약)
...
- C. revert : 해당 커밋의 변경사항을 취소하는 commit을 만드는 기능 (이력 관리를 위해서, 변경 사항을 되돌리는 커밋 만들기)
...
(원격저장소까지 푸시 했더라도 가능)
    ...
    ex) git add hello.py -> git commit
